...
const deployDaoAdditionFacet:DeployFunction = async function(hre: HardhatRuntimeEnvironment) {
   ...
   log("\n")
   const daoFacetDeployment = await deploy("DaoAdditionFacet", {
    from: deployer,
    args: [],
    log: true,
  ...
  const daoAdditionFacet = await ethers.getContract("DaoAdditionFacet") 
  ...
  const selectorsToAdd = getSelectors(daoAdditionFacet).get(['updateProposalList(uint256)', 'getProposalsList()'])
  const selectorToReplace = getSelectors(daoAdditionFacet).get(['sendLAR(address)'])
...
deployDaoAdditionFacet.tags = ["all", "daoAdditionFacet"]